...
export class TutorialControllerComponent implements OnInit {
  tutorialNodes: TutorialNode[] = [
    ...
    new TutorialNode(
      'Verifying the model',
      "You are now ready to verify that the interaction you have made is socially acceptable. Do so by clicking the 'Verify Model' button in the top-right corner. The model has three possible verification status's: Verified (green), Violating (red), and Not verified (orange). If the model is Verified, you can use the simulator in the bottom-right to see how the interaction will play out. If the model is Violating, you will need to fix the violations before you can use the simulator. If the model is Not verified, you will need to verify the model before you can use the simulator.",
      'assets/croppedRobotImages/neutral.png'
    ),
    new TutorialNode(
      'Fixing violations',
      "You'll notice the verification status is red which means the model contains violations. The violations are shown in the Violations panel on the bottom. Hover over the 'Farewell Flub' to see what micros are causing the flub. 'Greeter' only has one outgoing transition, but every micro needs two outgoing transitions. Add the Not Ready transition from 'Greeter' to 'Remark' and reverify the model.",
      'assets/tutorialGifs/10.gif'
    ),
    new TutorialNode(
      'More violations and microinteraction properties',
      "We go rid of the 'Farewell Flub' but now we have a 'Turn-taking Flub'. This means that the robot could speak twice in a row while the human is not ready. Hover over the violation to see the micros and transitons involved in the violation. To fix this, select the 'Greeter' micro and update the 'Wait for Response' property to 'Yes'. Reverify the model to see that the violations are gone.",
      'assets/tutorialGifs/11.gif'
    ),
    new TutorialNode(
      'Simulating the interaction',
      "Now that the model is verified you can use the simulator in the bottom-right. Everytime you make a change to the model you will need to reverify it to use the simulator. Click 'Play' to start the simulation.",
      'assets/croppedRobotImages/neutral.png'
    ),
    new TutorialNode(
    ...
    new TutorialNode(
      ...
      "You will notice the robot isn't saying anything when it reaches the 'Remark' micro. To fix this, select the 'Remark' micro and type something into its content box in the Inspector panel. Reverify the model and run the simulation again. The robot should now say your message when it comes to the 'Remark' micro.",
